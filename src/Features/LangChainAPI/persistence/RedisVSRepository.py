...
class RedisVSRepository:
    ...
    def __init__(self, embedding_factory: type[EmbeddingFactory] = None):
    ...
    async def delete_documents_by_metadata(self, metadata: dict):
        ...
        try:
            ...
            return
        # ============================================================
        # SEARCH BY SOURCE - DÙNG ĐỂ DEBUG
        # ============================================================
    from redisvl.query import FilterQuery

    async def search_by_source(self, source: str, k: int = 10):
        try:
            log.info(f"[DEBUG] Tìm kiếm theo source: {source}")
            ...
            query = FilterQuery(
                ...
                return_fields=["id", "source", "chunk_index", "page_content", "metadata"],
                num_results=k
            )
            ...
            log.info(f"[DEBUG] Found {len(docs)} docs")
            return docs

        except Exception as e:
            log.error(f"search_by_source error: {e}")
            return []
    # ============================================================
